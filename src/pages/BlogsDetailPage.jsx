...
const BlogsDetailPage = () => {
  ...
  const { data: newsData } = useFetch("/data/who_news.json");

  const blogs = blogsData?.find((blogs) => generateSlug(blogs.title) === slug);
  ...
  const sanitizedContent = DOMPurify.sanitize(blogs.content);

  const breadcrumbsItems = [
    { label: "Home", to: "/" },
    { label: "Blogs News", to: "/blogs" },
    { label: blogs.title, to: "#" },
  ];
      ...
      <div className="grid grid-cols-12">
        <div className="w-full col-span-9 p-4">
          <div className="bg-white border rounded-2xl p-10">
            ...
            <h1 className="text-3xl font-semibold mb-4">{blogs.title}</h1>
            <p className="text-gray-600 mb-4">Published on: {blogs.date}</p>
            <img
              src={blogs.thumbnail}
              alt={blogs.title}
        ...
        <div className="w-full col-span-3 p-4">
          <RecommendedSection newsData={newsData} blogsData={blogsData} />
        </div>
      </div>
    </div>
  );
